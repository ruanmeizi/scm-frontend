<template>
  <div class="container zeroshearboard">
    ...
    zerosSerch() {
      ...
      businessserch(this.currentPage, this.pageSize, obj).then(response => {
        if (response.code === "0") {
          ...
          this.zeroshearList = response.data.content;
          this.zeroshearList.forEach(item => {
    ...
    searchclick() {
     ...
     popoverHandle(index, item, name, field) {
      ...
      switch (field) {
         case 'materialCode':
          // 物料编码
          meterialdetails(item.materialCode, "material", this.value1, item.productCode).then(res => {
            if (res.code === "0") {
