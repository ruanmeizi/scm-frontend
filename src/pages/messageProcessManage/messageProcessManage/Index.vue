<template>
  <div class="container messageProcess">
    <div class="commoncontent">
      <div class="actions_part clearfix">
        <div class="actions_wrap">
          ...
          <div class="display_ib mr10 mb10">
            <span class="tag">消息ID：</span>
            <el-input class="input_single w200 mr10" v-model.trim="messageID" placeholder="请输入"></el-input>
          </div>
          <div class="display_ib mr10 mb10">
...
<script>
import { constDictList, deadMessageSelect,deadMessageSend } from "@/api/module/messageProcessApi.js";
...
export default {
  ...
  methods: {
    getMessageSendStatus() {
      // 贸易条款列表
      constDictList("DeadMessageSendStatus").then(res => {
    ...
    popoverHandle(index, item, name, field) {
       switch (field) {
          case "jsonMessage":
          var data=JSON.parse(item.jsonMessage);
    ...
    deadMessageSelectList() {
      this.tableLoading = true;
      let obj = {
        jsonMessage: this.messageInfo,
        sendStatus: this.messageState,
        id: this.messageID
      };
      deadMessageSelect(this.currentPage, this.pageSize, obj).then(response => {
        if (response.code === "0") {
    ...
    resendHandle(index,item){
      ...
      deadMessageSend(item.id).then(res=>{
        if(res.code==='0'){
          var val,type;
          if(res.data.success){
            val='操作成功'
