<template>
  <div class="container ZCStockPlan">
    ...
    <div class="table_part clearfix">
      <el-tabs class="subnav_wrap" :class="fieldListCopy.length > 10 ? 'auto' : ''" v-loading='tableLoading'>
        ...
        <!-- <p v-if="(!purchaseList.length || !fieldListCopy.length) && !tableLoading" style="text-align: center;margin:20px 0;" class="">~~暂无数据~~</p> -->
      ...
      console.log('弹层数据：', index, item, name, field);
       switch (field) {
        case 'usableReadyTotal':
          ...
          break;
        case 'period':
          var backorder = [JSON.parse(item.periodDetail)];
          console.log(backorder);
          this.$store.dispatch('getPopoverList', backorder);
          this.$forceUpdate();
          break;
