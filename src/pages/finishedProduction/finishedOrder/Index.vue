<template>
  <div class="container finishedOrder">
...
export default {
  ...
  data() {
    return {
      ...
      childHeaderList: ['行号', '期望交期', '生产物料编号', '生产数量', '入库仓位', '入库库位', '已入库数', '备注', '原材料物料编号', '原材料类型', '原材料消耗数量', '供应商', '出库仓位', '出库库位'],
      childFieldsList: ['sn', 'requiredDate', 'productionMaterialCode', 'productionQuantity', 'inVirtualInventoryName', 'inSubInventoryName', 'inventoryInNum', 'remark', 'sourceMaterialCode', 'materialTypeName', 'sourceQuantity', 'supplierShortName', 'outVirtualInventoryName', 'outSubInventoryName'],
  ...
  methods: {
    ...
    getCheckoutSubInventory(){
      return subInventoryTypeList('B0').then(res => {
        if (res.code === '0') {
          ...
          if(this.outSubInventoryList.findIndex(target=>target.subInventory==="18")==-1){
              console.log('验证不通过字典返回不存在默认出库库位')
              return false;
          }else {
            console.log('验证通过字典返回存在默认出库库位')
             return true;
          }
        }
      });
    ...
    async saveEditHandle() {
      ...
      this.submitData.productionOrderType = 'finishedProduct';
      // this.submitData.submitType = '1';
    ...
    getFabricProduceList() {
      ...
      let data = {
        ...
        productionOrderType:'finishedProduct'
      };
    ...
    getMaterialClassList() {
      // 生产类型
  		constDictList('FinishedProductProduction').then(res => {
...
<style>
.finishedOrder .subnav_wrap .el-tabs__item {
  height: 40px !important;
  line-height: 40px !important;
  font-size: 14px !important;
  vertical-align: baseline !important;
}
.finishedOrder .subnav_wrap.el-tabs--top .el-tabs__item.is-top:nth-child(2) {
  padding-left: 0;
}
.finishedOrder .subnav_wrap .el-tabs__content {
  overflow: visible;
}
.finishedOrder .subnav_wrap .checkbox_list .el-checkbox__label {
  width: 100%;
  padding-left: 0;
  color: #606266;
}
.finishedOrder .subnav_wrap .checkbox_list .el-checkbox {
  width: 100%;
}
.finishedOrder .subnav_wrap .checkbox_list .el-checkbox__input {
  margin-right: 10px;
}
</style>